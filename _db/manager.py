...
class DBManager:
    ...
    def get_rows(self, table_name: str, select:str="*"):
        """
    ...
    def row_count(self, table_name: str) -> int:
        ...
        """
        try:
            r = self._con.execute(f"SELECT COUNT(*) FROM {table_name}").fetchone()
            count = r[0] if r else 0
            db_log("info", f"[duck] row_count: {table_name} = {count}")
            return count
        except Exception as e:
            db_log("error", f"[duck] row_count: err {table_name}", error=str(e))
            return 0
